...
def test_check_in_morning(
    ...
    mock_coach.get_morning_coaching.assert_called_once()
...
def test_check_in_evening(
    ...
    mock_coach.get_evening_coaching.assert_called_once()
    mock_session_logger.log_interaction.assert_called_once()
    mock_session_logger.end_session.assert_called_once_with("test_session")
...
def test_task_add(runner, mock_data_store, mock_coach, mock_prompt_builder, mock_session_logger):
    ...
    result = runner.invoke(
        app, ["task", "--action", "add"], input="Test Task\nTest Description\nhigh\n2024-03-20\ny\n"
    )
...
def test_project_add(runner, mock_data_store, mock_session_logger):
    ...
    result = runner.invoke(
        app, ["project", "--action", "add"], input="Test Project\nTest Description\n"
    )
...
def test_analyze_patterns(runner, mock_context_manager, mock_session_logger):
    ...
    mock_context_manager.analyze_productivity_patterns.return_value = {
        ...
        "goal_progress": {
            "Test Goal": {"completed_tasks": 3, "total_tasks": 5, "journal_mentions": 2}
        },
    }
...
def test_prompt_adaptation():
    ...
    assert (
        effectiveness["morning_prompt"]["current_score"] < 0.7
    )  # Should be low due to unclear tasks
    ...
    assert (
        effectiveness["evening_prompt"]["current_score"] < 0.7
    )  # Should be low due to shallow reflections
    ...
    assert (
        effectiveness["task_breakdown_prompt"]["current_score"] < 0.7
    )  # Should be low due to vague subtasks
...
def test_prompt_builder_adaptation():
    ...
    changes = {
        "morning_prompt": {
            ...
            "issues": ["task_clarity"],
            "suggested_changes": {
                "task_clarity": {"add": ["New prompt element"], "remove": ["Old prompt element"]}
            },
        }
    }
...
class MockContextManager:
    ...
    def get_recent_context(self, days=7):
        return {"tasks": [], "check_ins": [], "journal_entries": []}
...
def test_feature_cli(monkeypatch):
    ...
    result = runner.invoke(
        app, ["feature", "add"], input="New Feature\ny\nNew Feature\nmedium\ntest,cli\n"
    )
    assert result.exit_code == 0
